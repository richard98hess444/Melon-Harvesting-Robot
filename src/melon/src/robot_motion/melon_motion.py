...
def main():
    ...
    try:
        while(not rospy.is_shutdown()):
            ...
            cameraSteady = False
            while(not cameraSteady):
                if melon.mz == 0 or melon.mz>1:
                    print('invalid depth')
                    rospy.sleep(1)
                else:
                    cameraSteady = checkMelonSteady(melon)
            print('steady camera')

            # Get the Euclidean coordinate of the melon under camera frame
            p_melon_C = np.array([[melon.mx, melon.my, melon.mz]]).T
            p_eef_C = p_melon_C - np.array([[0,0,0.2]]).T
            print('Radius:', melon.mr)

            # Calculate the world coordinate of the melon
            q_now = tutorial.check_joint_state()
            x_now = tutorial.check_position()
            baseTeef = tmtf.poseMtx(x_now)
            p_eef_B = baseTeef @ eefTcof @ homoColumn(p_eef_C)
            p_eef_B = np.append(homoColumnInv(p_eef_B), x_now[3:])
            p_melon_B = baseTeef @ eefTcof @ homoColumn(p_melon_C)
            p_melon_B = np.append(homoColumnInv(p_melon_B), x_now[3:])

            # # Create a virtual object and move the maniulator
            tutorial.add_obj('target', p_melon_B, 'sphere', melon.mr)
            ...
            tutorial.add_obj('plane_right', 
                             ...
                             'box', melon.mr, box_size=(0.8, 0.001, 1.6))
            x = input()
            tutorial.set_pose_goal(p_eef_B)
            tutorial.remove_box('target')
